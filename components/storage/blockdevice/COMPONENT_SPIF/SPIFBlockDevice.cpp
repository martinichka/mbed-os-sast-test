...
#define IS_MEM_READY_MAX_RETRIES  MBED_CONF_SPIF_DRIVER_MEM_READY_MAX_RETRIES
...
SPIFBlockDevice::SPIFBlockDevice(
    ...
    : _spi(mosi, miso, sclk), _cs(csel), _read_instruction(0), _prog_instruction(0), _erase_instruction(0),
      ...
      _is_enabled(false), _disabled_mode(EDisabledMode_t::AWAKE)
{
...
void SPIFBlockDevice::set_disabled_mode(SPIFBlockDevice::EDisabledMode_t mode)
{
    MBED_ASSERT(_is_initialized && !_is_enabled);
    _mutex->lock();

    if ((_disabled_mode == EDisabledMode_t::SLEEP) && (mode == EDisabledMode_t::AWAKE))
    {
        wake(); // SLEEP -> AWAKE
    }
    else if ((_disabled_mode == EDisabledMode_t::AWAKE) && (mode == EDisabledMode_t::SLEEP))
    {
        sleep(); // AWAKE -> SLEEP
    }
...
// Added by TWTG
void SPIFBlockDevice::enable(void)
{
    MBED_ASSERT(!_is_enabled);
    if (_disabled_mode == EDisabledMode_t::SLEEP)
    {
        wake();
    }
    _is_enabled = true;
...
// Added by TWTG
void SPIFBlockDevice::disable(void)
{
    MBED_ASSERT(_is_enabled);
    if (_disabled_mode == EDisabledMode_t::SLEEP)
    {
...
bool SPIFBlockDevice::is_enabled(void)
{
    return _is_enabled;
}
...
// Added by TWTG
void SPIFBlockDevice::sleep(void)
{
    _spi_send_general_command(0xB9, SPI_NO_ADDRESS_COMMAND, NULL, 0, NULL, 0);
    wait_us(10 + 50); // tDP + tDPDD
}

// Added by TWTG
void SPIFBlockDevice::wake(void)
{
    // 0xAB works for most microcontrollers
    _spi_send_general_command(0xAB, SPI_NO_ADDRESS_COMMAND, NULL, 0, NULL, 0); 
    wait_us(45); // tRDP 45us
}
...
int SPIFBlockDevice::_sfdp_detect_best_bus_read_mode(uint8_t *basic_param_table_ptr, int basic_param_table_size,
                                                     int &read_inst)
{
    (void) basic_param_table_ptr;
    (void) basic_param_table_size;
